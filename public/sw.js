const CACHE_VERSION = '20220614.28';
...
const APP_SHELL = ['/', '/auth/login', '/manifest.json', '/js/app.js'];
...
const APP_SHELL_INMUTABLE = [
  ...
  'https://unpkg.com/dexie@3.0.3/dist/dexie.js',
];
...
self.addEventListener('fetch', (event) => {
  ...
  if (event.request.method === 'POST') {
    if (!CACHE_ROUTES.some((route) => event.request.url.includes(route))) {
      ...
      logFetch(event.request, respuesta, 'request found in network: ');
    ...
    respuesta = cacheMatch(event.request.clone(), db.post_cache).then((response) => {
      if (response) {
